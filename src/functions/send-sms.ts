...
const getMyLinkMessages = (
  payloadMessage: PayloadSmsMessage,
  obfuscation: MyLinkSmsMessageObfuscateOptions,
  hasScheduledIn: boolean,
  ...
  return payloadMessage.receivers.map((receiver): MyLinkSmsMessage => {
    ...
    if (Object.keys(validationErrors).length !== 0) {
      ...
      throw new HTTPError(400, "MyLink SMS message validation failed", JSON.stringify(validationErrors));
    }
...
export async function sendSms(request: HttpRequest, _: InvocationContext): Promise<HttpResponseInit> {
  ...
  logger.info(
    "{Action} {MessageCount} SMS message(s) {Obfuscation}{ScheduledString}",
  ...
  try {
    ...
    logger.info(
      "{Action} {MessageCount} SMS message(s){ScheduledString}. Response: {Response}",
      hasScheduledIn || hasScheduledAt ? "Scheduled" : "Sent",
      myLinkSmsData.length,
      logScheduledString,
      JSON.stringify(response.messages)
    );
    countInc(`${MetricsPrefix}_${MetricsFilePrefix}_count`, "Number of SMS sent to Provider", myLinkSmsData.length, [
      MetricsResultLabelName,
    ...
    logger.errorException(
      error,
      "Failed to {Action} {MessageCount} SMS message(s){ScheduledString}. Payload: {Payload}",

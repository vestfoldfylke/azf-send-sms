...
export async function DeleteAsync(url: string): Promise<void> {
  ...
  if (!response.ok) {
    ...
    count(`${MetricsPrefix}_${MetricsFilePrefix}_DeleteRequest`, "Number of DELETE requests to MyLink", [
...
export async function GetAsync<T>(url: string): Promise<T> {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    count(`${MetricsPrefix}_${MetricsFilePrefix}_GetRequest`, "Number of GET requests to MyLink", [
      MetricsResultLabelName,
      MetricsResultFailedLabelValue
    ]);
...
export async function PostAsync<T>(url: string, body: string): Promise<T> {
  ...
  if (!response.ok) {
    const errorData = await response.json();
    count(`${MetricsPrefix}_${MetricsFilePrefix}_PostRequest`, "Number of POST requests to MyLink", [
      MetricsResultLabelName,
      MetricsResultFailedLabelValue
    ]);
